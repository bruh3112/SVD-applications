...
def display_original_and_compressed_images(original_img, compressed_img):
    cv2_imshow(original_img)
    cv2_imshow(compressed_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

def channel_svd_manual(channel):
    """
    Manual calculation of SVD for the input data matrix.
    :param channel: data matrix whose SVD is to be calculated
    :return: list of three matrices: U, Sigma, and V transpose
    """
    m, n = channel.shape
    # Calculate covariance matrix
    covariance_matrix = np.dot(channel, channel.T)
    ...
    U = eigenvectors
    Sigma = np.diag(np.sqrt(np.abs(eigenvalues)))
    Vt = np.dot(np.linalg.pinv(Sigma), np.dot(U.T, channel))

    return [U, Sigma, Vt]

img = cv2.imread(str(sys.argv[1]))
img = cv2.imread('/content/anh.jpg')
...
display_original_and_compressed_images(img, re_image)

# Measure time for library version
start_time_library = time.time()
[u_red, s_red, vt_red] = channel_svd(red_channel)
end_time_library = time.time()

# Measure time for manual version
start_time_manual = time.time()
[u_red_manual, s_red_manual, vt_red_manual] = channel_svd_manual(red_channel)
end_time_manual = time.time()

# Display time comparison
print("Time taken for library SVD:", end_time_library - start_time_library, "seconds")
print("Time taken for manual SVD:", end_time_manual - start_time_manual, "seconds")